...
5. Strømmetjenester (`StreamingProvidersList`): én badge per tjeneste som tilbyr tittelen (flere tilbudstyper for samme `providerId` slås sammen, aldri flere badger). Er tittelen inkludert i abonnementet eller gratis hos tjenesten (`type: "subscription"` eller `"free"`), vises kun tjenestens logo — ingen synlig navnetekst, ingen betalingslabel. Krever tittelen ekstra betaling (kun `"buy"` og/eller `"rent"`, uten abonnement/gratis), vises i tillegg en liten betalingslabel under logoen: «Kjøp», «Leie» eller «Kjøp/leie». Mangler tjenesten `logoUrl`, vises tjenestenavnet som tekst i stedet for logo (betalingslabel vises fortsatt ved behov). Når `streaming` er `null` eller `offers` er tom, vises en tom-tilstand («Ingen strømmetjenester funnet for din region»). Lenker til strømmetjenester åpnes i ny fane med `target="_blank"` og `rel="noopener noreferrer"`; kun `https:`-URL-er rendres (validert i provider-laget, se [architecture.md](./architecture.md#robusthet-og-sikkerhet)).
...
- **Wordmark/tema-navn «Streamie»** er en del av hifi-designet (gradient-tekst-logo på søkesiden) og tas i bruk som appens visuelle identitet. Prosjektnavnet er endret fra `watchlist` til `streamie` (repo, `package.json`); rutene (`/`, `/mylist`, `/title/:id`, `/feedback`) er uendret.
